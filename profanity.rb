...
class PercWindow < Curses::Window
  ...
  @ALL_SPELLS = {
	...
	def add_string(text, line_colors)
		...
		if @ALL_SPELLS.include?(spell_name.strip)
...
Thread.new {
	begin
		...
		handle_game_text = proc { |text|
			...
			if current_stream.nil? or stream_handler[current_stream] or (current_stream =~ /^(?:death|logons|thoughts|voln|familiar|assess|ooc|shopWindow|combat)$/)
				SETTINGS_LOCK.synchronize {
			...
			unless text.empty?
				if current_stream
					...
					elsif current_stream =~ /^(?:death|logons|thoughts|voln|familiar|assess|ooc|shopWindow|combat)$/
						if window = stream_handler['main']
